...
question_codes_2 <- c("ENGAGEMENT2_Q1", "ENGAGEMENT2_Q2", "ENGAGEMENT2_Q3", "ENGAGEMENT2_Q4",
                      "EQUITY2_Q1", "EQUITY2_Q2", "EQUITY2_Q3",
                      "REENTRY2_Q1", "REENTRY2_Q2A", "REENTRY2_Q2B", "REENTRY2_Q3")

# weighting_impact <- rbind(
#   analysis_dataset_2 %>% 
#     summarize(across(all_of(question_codes_2), mean, na.rm = T)),
#   analysis_dataset_2 %>%
#     summarize(across(all_of(question_codes_2), ~weighted.mean(., w = wt, na.rm = T))),
#   analysis_dataset_2 %>%
#     summarize(across(all_of(question_codes_2), sd, na.rm = T))
# 
# analysis_dataset_2 %>% group_by(AGENCY) %>% summarise(across(all_of(question_codes_2),
...
for(a in unique(analysis_dataset_2$AGENCY)){
  print(paste0("Calculating weighted and unweighted averages for ", a))
  agency_avg <- get_agency_weighted_averages(analysis_dataset_2, a) %>%
    mutate(agency = a) %>%
    relocate(agency)
...
for (a in unique(analysis_dataset_2$AGENCY))
{
  ...
  agency_df0 <- analysis_dataset_2 %>%
    filter(AGENCY == a)
  agency_df0$weights_col <- get_agency_weights(agency_df0)
  
  for (q in question_codes_2){
...
ui_a <- navbarPage("Pulse 2 Descriptive Analysis by Agency",
                              ...
                              sidebarLayout(
                                sidebarPanel(
                                  ...
                                  selectInput("Question",
                                              ...
                                              list('Engagement' = c(engagement1, engagement2, engagement2),
                                                   'Reentry' = c(reentry1, reentry2, reentry2),
                                                   'Equity and Inclusion' = c(equity1, equity2, equity4),
                                                   'FEVS Question' = c(equity2, ""))),
...
#       #geom_line() +
#       geom_errorbar(aes(ymin=lower.CL, ymax=upper.CL), width=0.4, color='orange', alpha = 0.9, size = 1.2) +
...
agency_avg_df %>% filter(agency == "General Services Administration") %>% 
  left_join(stack(q_map_2), by = c("model" = "values")) %>%
  rename(question_text = ind) %>% arrange(agency, model, metric, variable) %>%
  write.csv("D:/nmiller/pulse_2_analysis/GSA_demographic_breakouts_pulse_2.csv", row.names = F)

agency_emmeans_df %>% filter(agency == "General Services Administration") %>% 
  left_join(stack(q_map_2), by = c("model" = "values")) %>%
  rename(question_text = ind) %>% arrange(agency, model, variable, variable_levels) %>%
  write.csv("D:/nmiller/pulse_2_analysis/GSA_demographic_breakouts_emmeans_pulse_2.csv", row.names = F)

agency_avg_df %>%
  left_join(stack(q_map_2), by = c("model" = "values")) %>%
  rename(question_text = ind) %>% arrange(agency, model, metric, variable) %>%
  write.csv("D:/nmiller/pulse_2_analysis/agency_pulse_2_means.csv", row.names = F)

agency_emmeans_df %>%
  left_join(stack(q_map_2), by = c("model" = "values")) %>%
  rename(question_text = ind) %>% arrange(agency, model, variable, variable_levels) %>%
  write.csv("D:/nmiller/pulse_2_analysis/agency_pulse_2_emms.csv", row.names = F)